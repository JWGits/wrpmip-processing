...
def plotnine_scatter_delta(f, config, out_dir): 
    ...
    p2.save(filename=file_name+'_monthly.png', path=config['output_dir']+'combined/'+out_dir, \
        height=5, width=8, units='in', dpi=300)
## scatter plots
#def graph_scatter(f, config_files, out_dir):
#    # read in inputs
#    sites = f[0]
#    var = f[1]
#    models = f[2]
...
#    combined_file = config['output_dir'] + 'combined/WrPMIP_all_models_sites_2000-2021.zarr'
#    ds = xr.open_zarr(combined_file, use_cftime=True, mask_and_scale=True)
#    with open(Path(config['output_dir'] + 'combined/graph_lines_debug.txt'), 'w') as pf:
#        print('opened zarr file',file=pf)
#        print(ds, file=pf)
#    # subsample data
#    ds = ds[var].sel(site=sites, model=models, sim=sims)
#    # deal with variable site/model/sims and variable depth increments
#    listed = False
#    if isinstance(var, str) & isinstance(models, str) & isinstance(sites, str) & isinstance(sims, str):
#        file_name = var + '_by_time_' + models + '_' + sites + '_' + sims
#    if isinstance(sites, list):
#        file_part = models + '_' + '-'.join(str(sites)) + '_' + sims 
#        listed = True
#    if isinstance(models, list):
#        file_part = '-'.join(str(models)) + '_' + sites + '_' + sims
#        listed = True
#    if isinstance(sims, list):
#        file_part = models + '_' + sites + '_' + '-'.join(str(sims))
#        listed = True
#    if listed == True:
#        file_name = var + '_by_time_' + file_part
#    # start figure plot 
...
#    with open(Path(config['output_dir'] + 'combined/graph_lines_debug.txt'), 'w') as pf:
#        print('started figure plot',file=pf)
#        print(ds, file=pf)
#    # set custom color maps
#    ax = plt.gca()
#    plot_colors = ['orange','purple','green','olive','red','cyan','magenta','yellow','brown','gray','black']
#    custom_cycler = cycler(color=plot_colors)
#    ax.set_prop_cycle(custom_cycler) 
#    # plot ER by time for all models
#    if isinstance(models, list):
#        color_iter = 0
#        for model in models:
#            # search through config files for model.json name and read config file
#            for con in config_files:
#                if model+'.json' in con: 
#                    config = read_config(con)
#                    break
#            # plot each line based on the surface soil depth for each model
#            ds.sel(SoilDepth = config['soil_depths'][0], method = 'nearest').plot(color=plot_colors[color_iter]).scatter(x='SoilTemp', y='TotalResp')
#            # update color iterator
#            color_iter += 1
#    else:
...
#                    config = read_config(con)
#                    break
#        # call plot once, with colors updating internally for other lists (site, sims)
#        ds.sel(SoilDepth = config['soil_depths'][0], method = 'nearest').plot().scatter(ax=ax, x='SoilTemp', y='TotalResp')
#    # save figure
#    plt.savefig(config['output_dir']+'combined/'+ out_dir + '/'+ file_name)
#    # close figure
#    plt.close(fig)

## load combined model, simulation, site files and plot
#def graph_lines(f, config_files, out_dir):
#    # read in inputs
#    sites = f[0]
#    var = f[1]
#    models = f[2]
